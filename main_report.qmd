...
subtitle: "design `r params$design_version`"
author: "A.G."
format: 
 html:
   self-contained: true
...
params:
  ...
  backbone_prefix: "Backbone_"
  prefix_chr1: "Backbone_Chr1"
  prefix_chr17: "Backbone_Chr17"
---
...
files_found_full_paths <- list_bed_files(dir_with_depth, ".regions.bed.gz")
...
targets_no_bb <- targets[!(grepl(paste0("^", params$backbone_prefix), targets))]
...
bed_annotated_backbone <- left_join(bed_joined, targets_annotated_parsed,
  by =
    "X4"
) |>
  distinct() |>
...
if (length(fusion_index) == 0) {
  ...
  for (i in fusion_index) {
    res <- knitr::knit_child("includes/_make_sample_boxplot.qmd", quiet = TRUE)
    cat(res, sep = "\n")
  }
}
```
...
# plot_backbone(paste0(params$backbone_prefix, "Chr1"))
plot_backbone(params$prefix_chr1)
```
...
plot_backbone(params$prefix_chr17)
```
...
# read run IDs table
# run_IDs there have diferent style - fix this
...
bed_annotated_runID <- bed_annotated |>
  mutate(sample = str_remove(sample, "_[TN]$")) |>
  join_bed_run_ids(run_ids_table)
